...
struct xml_reader_s {
    ...
    size_t tabsize;                 ///< Expand TAB character to this position
    xmlerr_loc_t curloc;            ///< Current reader's position
    xmlerr_loc_t prodloc;           ///< Position for reporting current production
    xmlerr_loc_t errloc;            ///< Reader's position at the beginning of last token
...
#define xml_reader_message_lastread(h, ...) \
        xml_reader_message(h, &h->errloc, __VA_ARGS__)
...
xml_reader_new_internal(xml_reader_t *master, strbuf_t *buf, const char *location)
{
    ...
    h->tokenlen = 0;
    h->errloc = h->curloc; // Shares reference to copy of location
...
xml_read_until_parseref(xml_reader_t *h, const xml_reference_ops_t *refops, void *arg)
{
    ...
    while (true) {
        do {
            xml_read_production_start(h, NULL); // Don't read anything, just update location
...
xml_parse_STag_EmptyElemTag(xml_reader_t *h)
{
    ...
    while (true) {
        ...
        else if (had_ws && (xml_read_production_start(h, NULL), xml_read_Name(h) == PR_OK)) {
